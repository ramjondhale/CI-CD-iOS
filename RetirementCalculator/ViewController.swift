...
class ViewController: UIViewController {
    ...
    func calculateRetirementAmount(current_age: Int, retirement_age: Int, monthly_investment: Float, current_savings: Float, interest_rate: Float) -> Double {
        ...
        let monthly_rate = Double(interest_rate / 100 / 12)
        let total_investments = Double(monthly_investment) * ((pow(1 + monthly_rate, Double(months_until_retirement)) - 1) / monthly_rate)

        retirement_amount += total_investments
